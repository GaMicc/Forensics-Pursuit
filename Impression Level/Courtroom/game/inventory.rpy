...
init python:  
    ...
    evidence = {
        "Case_A_Anthropology_1": "The observed trauma is consistent with impact from a focused blunt force instrument with a rounded striking surface. No weapon was recovered from the scene, but the characteristics of the injury indicate it was caused by a tool with a concentrated point of impact, such as a hammer, mallet, or a similarly shaped instrument.",
        "Case_A_Anthropology_2": "Analysis of the fracture patterns on the skull suggests two impacts that occurred in close succession on the superior occipital bone, below the lambdoid suture. The fracture lines associated with the right-side impact dissipate into those from the left, indicating that the left-side impact likely occurred first. We cannot determine the exact time between these two impacts, only their relative sequence if they occurred around the same time.",
        ...
        "Case_A_Biology_2": "Epithelial cells were recovered from underneath Ana Konzaki's fingernails. Standard STR DNA profiling resulted in a mixed profile. Attempts to isolate a minor profile for comparison to known individuals were unsuccessful due to degradation.",
        "Case_A_Chemistry_1": "Postmortem toxicology analysis of Ana Konzaki’s blood detected an ethanol concentration of 0.02% BAC, which is consistent with light alcohol consumption. No other intoxicants were detected. While the ethanol level is unlikely to have significantly impaired her motor skills or cognitive function, it could still have had a minor effect on her coordination and judgment at the time of the incident.",
        "Case_A_Chemistry_2": "Microscopic analysis of Ana Konzaki's head wound reveals small embedded traces of metal, suggesting the object that caused the injury had a metal striking surface.",
        "Case_A_Psychology_1": "Edward Bartlett's criminal history indicates repeated occurrences of violence. His most recent charge was 6 months prior for assaulting an individual during a bar altercation. Following the latest charge, Bartlett was sentenced to probation with mandatory anger management counseling, indicating ongoing concerns about his violent behaviour.",
        "Case_A_Psychology_2": "Case A, Psychology, Evidence 2 Description",
        "Case_A_Identification_1": "Fingerprints on the collected metal water bottle's sticker, developed using the DFO method. The print seems consistent with the right index finger on Edward Bartlett's elimination set.n",
        "Case_A_Identification_2": "A partial shoeprint found in a pool of blood, leading away from the crime scene. It was identified as a men's size 10 Timberland boot, a type of shoe which Edward Bartlett has been seen wearing before.",
        "Case_B_Anthropology_1": "Jacob's autopsy revealed bruising on his neck, but the hyoid bone was not fractured. Small hemorrhages (petechiae) were found in his eyes and face",
        "Case_B_Anthropology_2": "Based on bloating, skin slippage, and insect activity, it is estimated that he had been dead for approximately 36-48 hours before being discovered.",
        "Case_B_Biology_1": "Jacob's stomach contents included partially digested fast food, placing his last meal approximately 30 minutes to 1 hour before death.",
        "Case_B_Biology_2": "Jacob's fingernails had scrapings of skin cells that could not be associated with Kiernan. The DNA profile suggests multiple contributors, but none were in the criminal database.",
        "Case_B_Chemistry_1": "The water in Jacob's lungs and airways is not the same as the water from the lake. The chemical traces in the water from Jacob's lungs seem to be tap water, not lake water.",
        "Case_B_Chemistry_2": "Identified microscopic synthetic fibers on Jacob's clothing that match the carpet fibers from Kiernan DeSouza's car; however, it cannot be determined whether the fibers are innocuous or not because Jacob was dropped off at school in that car that morning.",
        ...
        "Case_B_Psychology_2": "Kiernan does not have any history of violence or violence-inducing disorders in his medical records. The only mental health diagnosis he has is ADHD.",
        "Case_B_Identification_1": "One of Jacob's shoes, a men's size 6 sneaker, was found at his school. This means Jacob was at school and likely got taken from there to the park either before or after death.",
        "Case_B_Identification_2": "There were many footprints in the mud near the bank of the lake. One of the partial 3D impressions recovered seems to be Jacob's, but the others are unidentifiable.",
    }
                ...
                for item in inventory_sprites:
                    if item.visible == True:
                        if item.x <= x <= item.x + item.width and item.y <= y <= item.y + item.height:
                            renpy.show_screen("inventoryItemMenu", _layer="ui", item = item)
                            renpy.restart_interaction()
                            break
                        else:
                            renpy.hide_screen("inventoryItemMenu", layer="ui")
                            renpy.restart_interaction()
...
screen inventory:
    ...
    add Solid("#000000a3")
    zorder 3
    image "UI/inventory2-bg.png" xpos 0.008 ypos 0.04 at Transform(xzoom=0.35, yzoom=0.5)
    image "UI/inventory-slots.png" xpos 0.04 ypos 0.1 at Transform(xzoom=0.5, yzoom=0.52)
...
screen inventoryItemMenu(item):
    ...
    frame:
        ...
        xpos int(item.x)
        ypos int(item.y)
        imagebutton auto "UI/view-inventory-item-%s.png" align (0.5, 0.5) at half_size action [Hide("case_description"), Show("inspectItem", _layer="ui", items = [item.type]), Hide("inventoryItemMenu", _layer="ui")]
